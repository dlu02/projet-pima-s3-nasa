...
const App = () => {
  ...
  const tendancesArray = async () => {
    var items = [];
    var json = await fetch("https://perso.dlu02.ovh/pima_S3/tendances.php").then(response => response.json());
  ...
  useEffect(() => {
    const applyFavoris = async () => {
      let items = await tendancesArray();
      // console.log(items);
      setDataSource(items);
    }
      applyFavoris();
      ...
      {modalVisibleStatus ? (
        ...
        <View style={styles.container}>
          <Text style={styles.titleStyle}>
            Tendances sur les 15 derniers jours
          </Text>
          <FlatList
            data={dataSource}
            renderItem={({item}) => (
